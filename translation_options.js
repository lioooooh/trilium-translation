 ...
 * @property {"static"|"dynamic"} method - 翻译方式
 */
...
for (const { method, selector, translation } of translationSettingList) {
    const nodeList = document.querySelectorAll(selector);
    switch (method) {
        case "static": {
            nodeList.forEach(node => recurveSearchNodeAndReplaceText(translation, node));
            break;
        }
        case "dynamic": {
            nodeList.forEach(node => watchAndTranslate(translation, node));
            break;
        }
    }
}
