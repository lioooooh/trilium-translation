...
$("button.tree-floating-button.bx.bx-cog.tree-settings-button").on('click', function(){
    ...
    console.log("1s翻译设定");
    setTimeout (()=>{doReplace(translation_tree_actions,node)}, 1000);
}); 
...
// 有时加载时就已经有页面打开，因此也需要 加载就绪后执行翻译， 
 $(".tab-row-widget.component").ready(function(){
   var node=".ribbon-container.component";
     ...
     setTimeout (()=>{ loopdoReplace(translation_note,node)}, 500);
    console.log("2s文档属性翻译设定");
    setTimeout (()=>{ loopdoReplace(translation_note,node)}, 2000);  
}); 
...
$(".tab-row-widget.component").on('click',function(){
   var node=".ribbon-container.component";
     console.log("0.5s文档属性翻译设定");
     setTimeout (()=>{ loopdoReplace(translation_note,node)}, 1000);
    console.log("2s文档属性翻译设定");
    setTimeout (()=>{ loopdoReplace(translation_note,node)}, 2000);  
}); 
